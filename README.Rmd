...
on top of the **mlr3 ecosystem**.
It implements a **gate-based workflow** (G0A/G0B, G1--G6, G7A/G7B) and assigns
...
`mlr3misc`, `data.table`, `checkmate`, and `R6`.
Optional integrations (pipelines, tuning, SHAP, DALEX/iml, plotting) are enabled when corresponding packages are installed.
...
claim-scoped IELs, and evidence-driven gate planning.
Default runs prioritize full auditing paths; quick mode is available only for iterative prototyping.
IEL-3 is reserved for declared high-stakes contexts and requires human-factors evidence
