...
test_that("export_audit_bundle emits reproducibility artifacts", {
  ...
  expect_true(all(c(
    "report_card", "report_card_extended", "guide_summary", "guide_actions",
    "trust_summary", "model_story", "reader_questions", "recommended_plots",
    "gate_results", "iel", "claim_scope", "traceability_status", "session_info"
  ) %in% names(files)))
